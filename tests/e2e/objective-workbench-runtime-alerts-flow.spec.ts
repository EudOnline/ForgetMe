...
test('objective workbench shows open alerts, budget pressure, and trend windows', async () => {
  ...
  try {
    ...
    await expect(page.getByText('Backlog new (24h)')).toBeVisible()
    await expect(page.getByText('Backlog resolved (24h)')).toBeVisible()
    await expect(page.getByText('Backlog net (24h)')).toBeVisible()
