...
test('memory workspace supports global, person, and group scoped asks', async () => {
  ...
  await page.getByLabel('Ask memory workspace').fill('那为什么这个冲突最值得先处理？')
  await page.getByRole('button', { name: 'Ask' }).click()
  ...
  await expect(page.locator('section[aria-label=\"Turn 2\"]')).toBeVisible()
  await expect(page.getByText('Conversation Context')).toBeVisible()
  const firstTurn = page.locator('section[aria-label=\"Turn 1\"]').first()
  await expect(firstTurn.getByText('Conflicts & Gaps')).toBeVisible()
  await firstTurn.getByRole('button', { name: 'Open school_name conflicts' }).first().click()
