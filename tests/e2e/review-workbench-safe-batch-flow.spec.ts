...
test('approves a safe profile batch and undoes it from review history', async () => {
  ...
  await page.getByLabel('Search history').fill('Alice Chen')
  await page.getByRole('button', { name: 'Filter History' }).click()
  await expect(batchRow.getByRole('button', { name: 'Replay' })).toBeVisible()
  await batchRow.getByRole('button', { name: 'Replay' }).click()
  await expect(page.getByText('Replay Detail')).toBeVisible()
  ...
  await page.getByLabel('Keyword').fill('Alice')
  await page.locator('form').getByRole('button', { name: 'Search' }).click()
  await expect(page.getByText('Decision History')).toBeVisible()
  await expect(page.getByText('Safe batch approve · Alice Chen · school_name · 2 items')).toBeVisible()
