...
async function seedPlannedSubagentObjective(userDataDir: string) {
  ...
  try {
    const started = await runtime.startObjective({
      title: 'Show subagent plan before tool execution',
      objectiveKind: 'evidence_investigation',
      prompt: 'Run a bounded external verification and expose the recorded execution plan.',
...
test('objective workbench shows a recorded subagent plan before execution details', async () => {
  const userDataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forgetme-objective-subagent-plan-e2e-'))
  await seedPlannedSubagentObjective(userDataDir)
  ...
  await expect(page.getByText('Show subagent plan before tool execution').first()).toBeVisible()
  await page.getByRole('button', { name: /Web verification/i }).click()
  await expect(page.getByText('Subagent plan recorded', { exact: true }).first()).toBeVisible()
  await page.getByRole('button', { name: 'Show full thread detail' }).click()
  await expect(page.getByText(/Execution plan for web-verifier:/i).first()).toBeVisible()
  await expect(page.getByText(/Search candidate sources/i).first()).toBeVisible()
