...
export type QueueItemRow = ReviewQueueItem & {
  candidateId: string
}

export type StructuredCandidateRow = StructuredFieldCandidate & {
  approvedJournalId: string | null
}

export type ProfileCandidateRow = ProfileAttributeCandidate & {
  approvedJournalId: string | null
...
export type SourceCandidateLookup = {
  candidateId: string
...
export function loadQueueItem(db: ArchiveDatabase, queueItemId: string): QueueItemRow {
...
export function loadStructuredCandidate(
  db: ArchiveDatabase,
  candidateId: string,
  queueItemId: string
): StructuredCandidateRow {
  const row = db.prepare(
    `select
      id,
      file_id as fileId,
...
export function loadProfileCandidate(
  db: ArchiveDatabase,
  candidateId: string,
  queueItemId: string
): ProfileCandidateRow {
...
export function loadFileContext(db: ArchiveDatabase, fileId: string | null) {
  if (!fileId) {
    return null
  }
...
export function loadEvidenceContext(db: ArchiveDatabase, evidenceId: string | null) {
  if (!evidenceId) {
    return null
  }
...
export function loadSourceCandidate(db: ArchiveDatabase, sourceCandidateId: string | null): SourceCandidateLookup | null {
...
export function loadDecisionJournalById(db: ArchiveDatabase, journalId: string | null) {
  if (!journalId) {
    return null
  }
...
export function loadLatestDecisionJournal(db: ArchiveDatabase, targetType: string, targetId: string) {
  const row = db.prepare(
    `select
      id,
      decision_type as decisionType,
...
export function resolveEvidenceIdFromJournal(journal: DecisionJournalEntry | null) {
  if (!journal) {
    return null
  }
...
export function resolveJournalForSourceCandidate(db: ArchiveDatabase, sourceCandidate: SourceCandidateLookup | null) {
  if (!sourceCandidate) {
    return null
  }
