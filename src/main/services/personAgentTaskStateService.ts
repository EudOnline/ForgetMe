...
type PersonAgentTaskExecutionRunDraft = Omit<
  PersonAgentTaskRunRecord,
  | 'runId'
  | 'createdAt'
  | 'updatedAt'
...
export function isAutoExecutablePersonAgentTask(task: PersonAgentTaskRecord) {
  return task.taskKind !== 'await_refresh'
}

export function buildPersonAgentTaskExecutionRunDraft(task: PersonAgentTaskRecord): PersonAgentTaskExecutionRunDraft {
...
export function transitionPersonAgentTaskState(db: ArchiveDatabase, input: {
  taskId: string
